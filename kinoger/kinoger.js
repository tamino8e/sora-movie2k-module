...
/*
 * Extrahiert Episoden aus allen möglichen Moflix-Datenstrukturen
 * (title.seasons, titlePage.seasons, titlePage.episodes etc.)
 */
function collectAllEpisodes(titlePage) {
  if (!titlePage) return [];

  var episodes = [];

  // 1. Moflix Standard: titlePage.title.seasons oder titlePage.seasons
  var seasons =
    (titlePage.title && titlePage.title.seasons) ||
    titlePage.seasons;

  if (Array.isArray(seasons) && seasons.length > 0) {
    for (var i = 0; i < seasons.length; i += 1) {
      var seasonObj = seasons[i];
      if (!seasonObj) continue;

      var seasonNum = toNumber(
        seasonObj.season_number || seasonObj.number || seasonObj.season
      );

      var seasonEps =
        seasonObj.episodes && Array.isArray(seasonObj.episodes.data)
          ? seasonObj.episodes.data
          : Array.isArray(seasonObj.episodes)
          ? seasonObj.episodes
          : [];

      for (var j = 0; j < seasonEps.length; j += 1) {
        var ep = seasonEps[j];
        if (!ep) continue;

        var epCopy = {};
        for (var key in ep) {
          if (Object.prototype.hasOwnProperty.call(ep, key)) {
            epCopy[key] = ep[key];
          }
        }

        if (!epCopy.season_number) {
          epCopy.season_number = seasonNum;
        }

        episodes.push(epCopy);
      }
    }
  }

  // 2. Fallback: Flache Episodenlisten
  if (!episodes.length) {
    var rawEps = [];
    if (titlePage.episodes && Array.isArray(titlePage.episodes.data)) {
      rawEps = titlePage.episodes.data;
    } else if (Array.isArray(titlePage.episodes)) {
      rawEps = titlePage.episodes;
    } else if (
      titlePage.title &&
      titlePage.title.episodes &&
      Array.isArray(titlePage.title.episodes.data)
    ) {
      rawEps = titlePage.title.episodes.data;
    } else if (
      titlePage.title &&
      Array.isArray(titlePage.title.episodes)
    ) {
      rawEps = titlePage.title.episodes;
    }

    episodes = rawEps.slice();
  }
...
async function extractEpisodes(url) {
  try {
    ...
    var episodes = collectAllEpisodes(titlePage);
    ...
    /*
     * Filtern und nach Season / Episode sortieren
     */
    var validEpisodes = [];
    for (var i = 0; i < episodes.length; i += 1) {
      var item = episodes[i];
      ...
      var season = toNumber(item.season_number || item.season || item.season_num);
      var episode = toNumber(item.episode_number || item.episode || item.episode_num || item.number);
    ...
    validEpisodes.sort(function(a, b) {
    ...
    for (var j = 0; j < validEpisodes.length; j += 1) {
      var epData = validEpisodes[j];
      var item = epData.item;
      var season = epData.season;
      var episode = epData.episode;

      var episodeTitle = cleanupText(item.name || item.title || "");
...
async function extractStreamUrl(url) {
  try {
    ...
    for (var i = 0; i < episodes.length; i += 1) {
      var item = episodes[i];
      var itemSeason = toNumber(item.season_number || item.season) || 1;
      var itemEpisode = toNumber(item.episode_number || item.episode || item.number) || 1;
    ...
    if (!watchUrl) {
      console.log(
        "Moflix watchUrl not found for S" +
          padNumber(season) +
          "E" +
          padNumber(episode)
      );
    ...
    var watchHtml = await fetchHtml(watchUrl);
    var watchBootstrap = parseBootstrapData(watchHtml) || {};
    var watchPage = getWatchPage(watchBootstrap);
    ...
    if (!resolved.length) {
      for (var j = 0; j < videoList.length; j += 1) {
...
async function moflixMultiExtractor(providers) {
  ...
  for (var url in providers) {
    ...
    var provider = String(providerValue || "").split("-")[0];
...
async function ensureWatchUrl(url) {
  ...
  if (episodes.length) {
    for (var i = 0; i < episodes.length; i += 1) {
      ...
      var itemSeason =
        toNumber(item.season_number || item.season) || 1;

      var itemEpisode =
        toNumber(item.episode_number || item.episode || item.number) || 1;

      if (
        itemSeason === season &&
        itemEpisode === episode
      ) {
        if (item.primary_video && item.primary_video.id) {
          return absolutizeUrl("/watch/" + item.primary_video.id);
        }

        if (item.video && item.video.id) {
          return absolutizeUrl("/watch/" + item.video.id);
        }

        if (item.video_id) {
          return absolutizeUrl("/watch/" + item.video_id);
        }

        if (Array.isArray(item.videos) && item.videos.length && item.videos[0].id) {
          return absolutizeUrl("/watch/" + item.videos[0].id);
        }

        if (item.id) {
          return absolutizeUrl("/watch/" + item.id);
        }
...
async function resolveMirror(url, depth) {
  ...
  try {
    var response = await moflixFetch(normalizedUrl);
    var html = await readResponseText(response);
...
function collectWatchVideos(watchPage) {
  ...
  if (Array.isArray(watchPage.alternative_videos)) {
    for (var i = 0; i < watchPage.alternative_videos.length; i += 1) {
  ...
  for (var j = 0; j < sources.length; j += 1) {
    var item = sources[j];
    var src = absolutizeUrl(item && item.src);
    ...
    combined.push({
      src: src,
      quality: (item && item.quality) || "Unknown"
    });
  }
...
function getMovieVideoId(titlePage) {
  ...
  if (
    Array.isArray(titlePage.title && titlePage.title.videos) &&
...
function getTitlePage(bootstrapData) {
  return (
    (bootstrapData.loaders && bootstrapData.loaders.titlePage) || {}
  );
}

function getWatchPage(bootstrapData) {
  return (
    (bootstrapData.loaders && bootstrapData.loaders.watchPage) || {}
  );
}

function getSearchPage(bootstrapData) {
  return (
    (bootstrapData.loaders && bootstrapData.loaders.searchPage) || {}
  );
}

function parseBootstrapData(html) {
  var marker = "window.bootstrapData = ";
...
function buildTitleHref(item) {
  ...
  var slugSource = item.name || item.original_title || "";
  var slug = slugifyTitle(slugSource);
...
async function fetchHtml(url) {
  var response = await moflixFetch(stripHash(url));
...
function isLunaRuntime() {
  return (
    typeof fetchV2Native === "function" ||
    typeof fetchNative === "function"
  );
}
...
function extractDirectUrlFromHtml(html) {
  ...
  for (var i = 0; i < patterns.length; i += 1) {
    var match = normalizedHtml.match(patterns[i]);

    if (!match) {
      continue;
    }

    var candidate = absolutizeUrl(
      decodeJsString(match[1] || match[0])
    );
  ...
  while ((scriptMatch = scriptRegex.exec(normalizedHtml)) !== null) {
    var scriptContent = String(scriptMatch[1] || "").trim();
    ...
    try {
      var unpackedScript = unpack(scriptContent);

      for (var j = 0; j < patterns.length; j += 1) {
        var unpackedMatch = unpackedScript.match(patterns[j]);

        if (!unpackedMatch) {
          continue;
        }

        var unpackedCandidate = absolutizeUrl(
          decodeJsString(unpackedMatch[1] || unpackedMatch[0])
        );

        if (unpackedCandidate && isDirectMediaUrl(unpackedCandidate)) {
...
function scoreStream(stream) {
  var score = 0;
  var link = String(stream.link || "").toLowerCase();
...
class Unbaser {
  constructor(base) {
    ...
    if (2 <= base && base <= 36) {
      ...
      for (var i = 0; i < this.ALPHABET[base].length; i += 1) {
  ...
  _dictunbaser(value) {
    ...
    for (var i = 0; i < chars.length; i += 1) {
      result +=
        Math.pow(this.base, i) * this.dictionary[chars[i]];
    }
...
function unpack(source) {
  ...
  return payload.replace(/\b\w+\b/g, function(word) {
    var index =
      radix === 1 ? parseInt(word, 10) : unbaser.unbase(word);
